	 ...
	 IMenuListener menuListener = new IMenuListener() {
			...
			public void menuAboutToShow(IMenuManager manager) {
				...
				if(manager instanceof MenuManager){
					try {
						TTSConversion.getDefault().speak(((MenuManager)manager).getMenuText());
					} catch (IOException e) {
...
//	org.eclipse.jface.action.MenuManager menu = (org.eclipse.jface.action.MenuManager) ((WorkbenchWindow)PlatformUI.getWorkbench().getActiveWorkbenchWindow()).getActionBars().getMenuManager();
//	
//	
	...
	public void add(IContributionItem item) {
		...
		if (allowItem(item)) {
			...
			if(item instanceof MenuManager){
				((MenuManager)item).addMenuListener(this.menuListener);
			}
			itemAdded(item);
		}
	}
